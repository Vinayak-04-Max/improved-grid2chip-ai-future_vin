...
export const VelocityText: React.FC<VelocityTextProps> = ({
  ...
  return <div className="overflow-hidden whitespace-nowrap flex flex-nowrap">
      <motion.div className={cn("flex whitespace-nowrap flex-nowrap", className)} style={{
      x
    }}>
        {[...Array(4)].map((_, i) => <span key={i} className="mr-fib-34">
            {children}
          </span>)}
      </motion.div>
    </div>;
...
export const OrbitingWheel: React.FC<OrbitingWheelProps> = ({
  ...
  const innerPulseAnimate = animated ? {
    scale: [1, 1.05, 1]
  } : undefined;
  ...
  return <div className={cn("relative flex items-center justify-center", className)} style={{
    height: radius * 2.8,
    width: radius * 2.8
  }}>
      {/* Center Content */}
      {animated ? <motion.div className="absolute z-10" animate={{
      scale: [1, 1.05, 1]
    }} transition={{
      duration: 4,
      repeat: Infinity,
      ease: "easeInOut"
          ...
          {centerContent}
        </motion.div> : <div className="absolute z-10">{centerContent}</div>}

      {/* Orbital Rings */}
      <motion.div className="absolute border border-primary/20 rounded-full" style={{
      width: radius * 2,
      height: radius * 2
    }} animate={ring1Animate} transition={ring1Transition} />
      <motion.div className="absolute border border-accent/10 rounded-full" style={{
      width: radius * 1.5,
      height: radius * 1.5
    }} animate={ring2Animate} transition={ring2Transition} />

      {/* Orbiting Items */}
      <motion.div className="absolute px-0" style={{
      width: radius * 2,
      height: radius * 2
    }} animate={orbitAnimate} transition={orbitTransition}>
        {items.map((item, index) => {
        const angle = index * angleStep - Math.PI / 2;
        const x = Math.cos(angle) * radius;
        const y = Math.sin(angle) * radius;
        return <motion.div key={item.name} className="absolute glass-panel rounded-xl p-3 md:p-4 cursor-pointer group shadow-lg" style={{
          left: '50%',
          top: '50%',
          x: x - 55,
          y: y - 40
        }} animate={itemAnimate} transition={itemTransition} whileHover={{
          scale: 1.1,
          zIndex: 50
        }}>
              <motion.div className="flex flex-col items-center gap-1.5 min-w-[90px] md:min-w-[100px]" animate={innerPulseAnimate} transition={animated ? {
...
export const FloatingObject: React.FC<FloatingObjectProps> = ({
  ...
  return <motion.div className={cn("", className)} animate={{
...
export const HorizontalScrollSection: React.FC<HorizontalScrollProps> = ({
  ...
  return <section ref={targetRef} className="relative h-[300vh]">
...
export const PulseAccent: React.FC<PulseAccentProps> = ({
  ...
  return <div className={cn("relative", className)}>
      <motion.div className={cn("absolute rounded-full blur-sm", colorClasses[color], sizeClasses[size])} animate={{
      scale: [1, 1.5, 1],
      opacity: [0.8, 0.3, 0.8]
    }} transition={{
      duration: 2,
      ...
      <motion.div className={cn("absolute rounded-full blur-md", colorClasses[color], sizeClasses[size])} animate={{
      scale: [1, 2, 1],
      opacity: [0.5, 0.1, 0.5]
    }} transition={{
      duration: 2,
...
export const RefractiveCard: React.FC<RefractiveCardProps> = ({
  ...
  return <motion.div ref={cardRef} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} style={{
    rotateX: smoothRotateX,
    rotateY: smoothRotateY,
    transformStyle: "preserve-3d"
  }} className={cn("relative overflow-hidden rounded-fib-xl glass-panel border border-primary/20", className)}>
      {/* Refractive Glare */}
...
export const ContinuousCarousel: React.FC<CarouselProps> = ({
  ...
  return <div className={cn("overflow-hidden", className)}>
      <motion.div className="flex gap-fib-21" animate={{
      ...
      x: {
        ...
        {duplicatedChildren.map((child, index) => <div key={index} className="flex-shrink-0">
            {child}
          </div>)}
      </motion.div>
    </div>;
...
export const MorphingBlob: React.FC<MorphingBlobProps> = ({
  ...
  return <motion.div className={cn("absolute rounded-full blur-3xl bg-gradient-radial", gradients[color], className)} style={{
    width: size,
    height: size
  }} animate={{
...
export const DataStream: React.FC<DataStreamProps> = ({
  ...
  return <div className={cn("absolute inset-0 overflow-hidden opacity-20 pointer-events-none", className)}>
      {streams.map(stream => <motion.div key={stream.id} className="absolute text-primary font-mono text-xs leading-relaxed" style={{
      left: `${stream.x}%`
    }} initial={{
      y: '-100%',
      opacity: 0
      ...
      ease: 'linear'
          ...
          {stream.chars.map((char, i) => <div key={i} style={{
        opacity: 1 - i * 0.05
      }}>
              {char}
            </div>)}
