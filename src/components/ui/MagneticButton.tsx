...
export const MagneticButton: React.FC<MagneticButtonProps> = ({
  ...
  return <motion.div ref={ref} animate={{
    ...
    stiffness: 150,
    damping: 15
  }} onMouseMove={handleMouseMove} onMouseEnter={() => setIsHovered(true)} onMouseLeave={handleMouseLeave} className={cn('relative rounded-fib font-medium uppercase tracking-wider inline-block', 'transition-all duration-300 ease-smooth', 'overflow-hidden cursor-pointer', sizeClasses[size], variantClasses[variant], className)}>
      {/* Glow effect on hover */}
      <motion.div className="absolute inset-0 bg-gradient-to-r from-primary/20 via-accent/20 to-primary/20 pointer-events-none" initial={{
      opacity: 0,
      scale: 0.8
    }} animate={{
      ...
      {/* Content */}
      <span className="relative z-10 flex items-center justify-center gap-fib-8 text-sm">
        {children}
      </span>
    </motion.div>;
};
