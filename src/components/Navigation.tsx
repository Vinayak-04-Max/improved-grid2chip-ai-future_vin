...
const Navigation = () => {
  ...
  const NavLink = ({
    ...
    isActive: boolean;
  }) => <Link to={to} className={cn("relative font-medium transition-all duration-300 link-underline text-base text-primary-foreground", isActive ? "text-primary" : "text-muted-foreground")}>
      {children}
    </Link>;
  const DropdownMenu = ({
    label,
    ...
    isActive: boolean;
      ...
      <button className={cn("flex items-center gap-1 font-medium transition-all duration-300 text-base text-primary-foreground", isActive ? "text-primary" : "text-muted-foreground")}>
        {label}
        <motion.span animate={{
        rotate: isOpen ? 180 : 0
      }} transition={{
